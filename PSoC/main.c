...
uint16 timerStart()
{
    ...
    TimerResetReg_Write(1); // Turn timer off
    CyDelay(5); // Wait some clock cycles
    TimerResetReg_Write(0); // Turn timer on     
    return Timer_ReadCounter(); // Return initial time (uint16)
}
uint16 timerGetTime(uint16 initTime)
{
    return initTime - Timer_ReadCounter(); // uint16
}
...
int main(void)
{
    ...
    uint8 const ACC_SCALE = 0;
    uint8 const GYRO_SCALE = 0;  
    ...
    for(;;)
    {
        ...
        if((status & BTUART_RX_STS_FIFO_NOTEMPTY) != 0u)
        {
            ...
            if(errorStatus == 0u)
            {
                ...
                initTime = timerStart(); // Reset timer and get initial time of bt request
            }
        }
        if( timerGetTime(initTime) > BT_TIMEOUT ) // Stop bt if no response within timeout
        {
        ...
        if( (btRequest & IMU_REQ) != 0 )
        {
            ...
            // Send each byte and calculate checksum
            ...
            BTUART_WriteTxData(imuChecksum); // Send checksum. A total of 2 header bytes, 21 data bytes and 1 checksum have been sent
            CyDelayUs(UART_BYTE_DELAY);
        }
    }
}
